...
import { type ProjectConfig, saveProject } from "../lib/project";
	...
	async run({ args }) {
		...
		if (!name && !auto) {
			const input = await p.text({
				...
				placeholder: "my-api",
				validate: (v) =>
					!v ? "Required" : /[^a-z0-9-]/.test(v) ? "Lowercase + hyphens only" : undefined,
			});
...
`,
		...
		if (!auto) {
			p.note(
				[`cd ${name}`, "bun install", "bun run dev", "", "# When ready:", "atlas deploy"].join(
					"\n",
				),
				"Next steps",
			);
