...
async function configureCloudflare(panel: PanelClient) {
	...
	try {
		...
		spinner.stop(
			result.cloudflareConfigured ? "Cloudflare configured ✓" : "Failed to verify token",
		);
	} catch (e) {
...
async function configureGitHubApp(panel: PanelClient) {
	...
	if (org.githubAppConfigured) {
		const overwrite = await p.confirm({
			message: "GitHub App already configured. Overwrite?",
			initialValue: false,
		});
	...
	const result = await new Promise<{
		...
		const server = Bun.serve({
			...
			async fetch(req): Promise<Response> {
				...
				if (url.pathname === "/") {
					const m: string = manifest.replace(
...
<script>document.getElementById("f").submit()</script>
				...
				if (url.pathname === "/callback") {
					...
					if (!res.ok) {
						...
						resolve(null);
						return new Response("<h1>❌ Failed to create GitHub App</h1>", {
...
export default defineCommand({
	...
	args: {
		...
		"cloudflare-account": {
			type: "string",
			description: "Cloudflare account ID (non-interactive)",
		},
	},
	async run({ args }) {
			...
			options: [
				{
					...
					hint: "creates app automatically via browser",
				},
