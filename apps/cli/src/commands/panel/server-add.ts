...
export default defineCommand({
	...
	async run({ args }) {
		...
		if (!panel) {
			console.error("Not connected. Run: atlas panel setup");
			return;
		}
		...
		const name =
			args.name ||
			((await p.text({ message: "Server name", placeholder: "production" })) as string);
		if (p.isCancel(name)) return;

		const host =
			args.host || ((await p.text({ message: "SSH host", placeholder: "root@1.2.3.4" })) as string);
		if (p.isCancel(host)) return;

		const domain =
			args.domain ||
			((await p.text({ message: "Base domain", placeholder: "myapp.com" })) as string);
		...
		try {
			...
			if (provision) {
				...
				p.note(
					"Provisioning takes 5-10 minutes.\nCheck status: atlas panel status",
