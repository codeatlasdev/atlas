...
export const runtimeTypeEnum = pgEnum("runtime_type", ["k3s", "swarm"]);
...
export const users = pgTable(
	...
	(t) => [uniqueIndex("user_github_id_idx").on(t.githubId)],
);
...
export const servers = pgTable(
	"servers",
	{
		...
		runtime: runtimeTypeEnum().default("k3s").notNull(),
	...
	(t) => [uniqueIndex("server_name_org_idx").on(t.name, t.orgId)],
);
...
export const projects = pgTable(
	...
	(t) => [uniqueIndex("project_slug_org_idx").on(t.slug, t.orgId)],
);
...
export const deploys = pgTable(
	...
	(t) => [index("deploy_project_idx").on(t.projectId)],
);
...
export const secrets = pgTable(
	...
	(t) => [uniqueIndex("secret_project_key_idx").on(t.projectId, t.key)],
);
