...
export function phase03Ingress(domain: string) {
	return `set -euo pipefail
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
...
export function phase04aPrometheus(domain: string) {
	return `set -euo pipefail
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
...
export function phase05ArgoCD(domain: string) {
	return `set -euo pipefail
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
...
export function phase06App(domain: string) {
...
export function phase07Tunnel(cfToken: string, cfAccount: string, domain: string) {
	return `set -euo pipefail
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
...
export function getProvisionPhases(opts: ProvisionOptions): ProvisionPhase[] {
	const phases: ProvisionPhase[] = [
		...
		{ name: "Traefik + cert-manager", script: phase03Ingress(opts.domain) },
	]

	if (!opts.skipMonitoring) {
		phases.push(
			{ name: "Prometheus + Grafana", script: phase04aPrometheus(opts.domain) },
	...
	if (!opts.skipArgocd) {
		phases.push({ name: "ArgoCD", script: phase05ArgoCD(opts.domain) })
	}

	if (opts.tunnel) {
		phases.push({
			name: "Cloudflare Tunnel",
			script: phase07Tunnel(opts.tunnel.cfToken, opts.tunnel.cfAccount, opts.domain),
		})
	}

	phases.push({ name: "Application namespace", script: phase06App(opts.domain) })
